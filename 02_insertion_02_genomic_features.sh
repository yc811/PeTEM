#!/bin/bash
# 02_insertion_02_genomic_features.sh
# Usage example:
# bash 02_insertion_02_genomic_features.sh \
#   -g gene.bed -c CDS.bed -5 5UTR.bed -e exon.bed -3 3UTR.bed \
#   -p promoter.bed -t TE.bed -n genome.bed 
...
# 必要參數檢查
if [[ -z $gene || -z $cds || -z $utr5 || -z $exon || -z $utr3 || -z $promoter || -z $te || -z $genome  ]]; then
  echo "Usage: bash 02_insertion_02_genomic_features.sh -g gene.bed -c CDS.bed -5 5UTR.bed -e exon.bed -3 3UTR.bed -p promoter.bed -t TE.bed -n genome.bed"
  exit 1
fi
...
Rscript 02_insertion_02_genomic_features.R \
  ...
  merge_2strands_intron.bed merge_2strands_3UTR.bed merge_2strands_promoter.bed merge_2strands_IGR.bed "$genome" 
